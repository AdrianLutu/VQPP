...
# --- CONFIGURATION ---
# Use the paths to the pickle files generated by your previous script
...
class CorrelationDataset(Dataset):
    def __init__(self, pickle_file):
        ...
        # Sequential loading for appended pickle files
        with open(pickle_file, "rb") as f:
            while True:
                try:
                    # Each load returns (matrix, score) tuple
    ...
    def __getitem__(self, idx):
        ...
        # matrix is likely [1, 512, 512] from our generation script
...
class CNNRegressor(nn.Module):
    def __init__(self):
        ...
        # After 4 pools, 512 -> 256 -> 128 -> 64 -> 32
        self.gap = nn.AdaptiveAvgPool2d((1, 1))
        self.fc1 = nn.Linear(512, 512)
        self.fc2 = nn.Linear(512, 1)

    def forward(self, x):
        x = self.pool(F.relu(self.conv1(x)))  # 256
        x = self.pool(F.relu(self.conv2(x)))  # 128
        x = self.pool(F.relu(self.conv3(x)))  # 64
        x = self.pool(F.relu(self.conv4(x)))  # 32
...
train_loader = DataLoader(train_dataset, batch_size=32, shuffle=True)  # Reduced batch size for 512x512
...
# 3. Simple Loop (for brevity, keeping one set of params)
