...
TRAIN_JSONL_PATH = "/home/eduard/Desktop/Research/Adrian/VQPP/VAST/top100/msrvtt_train_top100.jsonl"
VAL_JSONL_PATH = "/home/eduard/Desktop/Research/Adrian/VQPP/VAST/top100/msrvtt_val_top100.jsonl"
TEST_JSONL_PATH = "/home/eduard/Desktop/Research/Adrian/VQPP/VAST/top100/msrvtt_test_top100.jsonl"
...
def load_video_frames(video_path, num_frames_to_sample, training=True):
    ...
    for i in range(num_frames_to_sample):
        ...
        if training:
            idx = np.random.randint(start, end) if start < end else start
        else:
            idx = (start + end) // 2
        indices.append(idx)
...
def process_lines(lines, text_model, tokenizer, vision_model, pbar_desc="Processing"):
    ...
    for line in tqdm(lines, desc=pbar_desc):
        try:
            ...
            gt_id = data.get('gt')
            if gt_id is None or not isinstance(gt_id, str): continue
        ...
        with torch.no_grad():
            inputs = tokenizer(text=[query_text], return_tensors="pt", max_length=77,
                               padding='max_length', truncation=True).to(DEVICE)
            outputs = text_model(input_ids=inputs["input_ids"], attention_mask=inputs["attention_mask"])
            text_emb = (outputs[0] / outputs[0].norm(dim=-1, keepdim=True)).detach().cpu().numpy()
            ...
            for video_id in retrieved_ids[:MAX_VIDEOS_PER_QUERY]:
                if video_id in video_embedding_cache:
                    ...
                    v_path = os.path.join(VIDEO_FOLDER, f"{video_id}{VIDEO_EXTENSION}")
                    ...
                    video_emb_tensor = torch.mean(visual_output, dim=0, keepdim=True)
...
def run_split(input_path, output_path, text_model, tokenizer, vision_model, label):
    if not os.path.exists(input_path):
        print(f"ERROR: {label} file not found at {input_path}")
        return
...
if __name__ == "__main__":
    ...
    run_split(TRAIN_JSONL_PATH, TRAIN_OUTPUT_PATH, text_model, tokenizer, vision_model, "Train")
    run_split(VAL_JSONL_PATH, VAL_OUTPUT_PATH, text_model, tokenizer, vision_model, "Validation")
    run_split(TEST_JSONL_PATH, TEST_OUTPUT_PATH, text_model, tokenizer, vision_model, "Test")
