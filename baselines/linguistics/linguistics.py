...
avg_scores_r10_test = pd.read_csv("C:\\Users\\Adi\\Desktop\\VQPP\\VAST\\msrvtt_test.csv")[["Query", "Recall@10"]]
avg_scores_rr_test = pd.read_csv("C:\\Users\\Adi\\Desktop\\VQPP\\VAST\\msrvtt_test.csv")[["Query", "Reciprocal_Rank"]]

metrics = pd.DataFrame([])


metrics["count_synsets"] = avg_scores_r10_test["Query"].apply(
    count_synsets
)
metrics["count_words"] = avg_scores_r10_test["Query"].apply(
    count_words
)
metrics["avg_word_length"] = avg_scores_r10_test["Query"].apply(
    avg_word_length
)


metrics["average_proper_nouns"] = avg_scores_r10_test["Query"].apply(
    average_proper_nouns
)
metrics["average_number_acronyms"] = avg_scores_r10_test[
    "Query"
].apply(average_number_of_acronyms)
metrics["average_numerals_occurrence"] = avg_scores_r10_test[
    "Query"
].apply(average_numerals_occurrence)
metrics["average_conjunctions_per_word"] = avg_scores_r10_test[
    "Query"
].apply(average_conjunctions_per_word)
metrics["average_prepositions_per_word"] = avg_scores_r10_test[
    "Query"
].apply(average_prepositions_per_word)

correlation_results_p10 = calculate_all_correlations(
    avg_scores_r10_test["Recall@10"].to_list(),
    metrics["count_synsets"],
    metrics["count_words"],
    metrics["avg_word_length"],
    metrics["average_proper_nouns"].to_list(),
    metrics["average_number_acronyms"].to_list(),
    metrics["average_numerals_occurrence"].to_list(),
    metrics["average_conjunctions_per_word"].to_list(),
    metrics["average_prepositions_per_word"].to_list(),
)

print(correlation_results_p10)

correlation_results_mrr = calculate_all_correlations(
    avg_scores_rr_test["Reciprocal_Rank"].to_list(),
    metrics["count_synsets"],
    metrics["count_words"],
    metrics["avg_word_length"],
    metrics["average_proper_nouns"].to_list(),
